...
pub fn handle_job_withdrew(conn: &mut PgConnection, log: Log) -> Result<()> {
    ...
    let duration_removed = if rate != 0 {
        std::time::Duration::from_secs((amount as u64).saturating_div(rate as u64))
    } else {
        std::time::Duration::from_secs(0)
    };

    info!(id, ?rate, ?duration_removed, "duration removed");

    // target sql:
    // UPDATE jobs
    // SET
    //     balance = balance - <amount>,
    //     expires_at = expires_at - <duration_removed>
    // WHERE id = <id>
    // AND is_closed = false;
    let count = diesel::update(jobs::table)
        .set((
            jobs::balance.eq(jobs::balance.sub(amount)),
            jobs::expires_at.eq(jobs::expires_at.sub(PgInterval::from_microseconds(
                duration_removed.as_micros().clamp(0, i64::MAX as u128) as i64,
            ))),
        ))
        .filter(jobs::id.eq(&id))
    ...
    // VALUES (<block>, <idx>, <job>, <amount>, false, <tx_hash>, <timestamp>);
    diesel::insert_into(job_transactions::table)
        .values((
            ...
            job_transactions::is_deposit.eq(false),
...
mod tests {
    ...
    use crate::handlers::JOB_WITHDREW;
